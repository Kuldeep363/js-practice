...
function helper(s,i,num,sign){
    if(i>= s.length || isNaN(parseInt(s[i]))) return sign * num;
    num = num*10 + parseInt(s[i]);
    if(sign*num>=MAX_INT) return MAX_INT;
    else if(sign*num <= MIN_INT) return MIN_INT;
...
function myAtoi(s) {
    ...
    s = s.trim(); // remove leading and training space
    let i = 0, sign = 1;
    if(s[i] === "-" || s[i] === "+"){
        sign = s[i] === "-"?-1:1;
        i++;
